...
class ParentRemoteDataSourceImpl implements ParentRemoteDataSource {
  ...
  Future<ParentModel> getParentWithChildren(String parentId) async {
    try {
      final response = await dio.post(
        ...
        data: '"$parentId"', // As per Postman collection, ID is sent as a raw string
      ...
      if (response.data != null && response.data['isSuccess'] == true) {
        ...
        // Find child objects
        final childrenJson = dataList.where((e) => e['role'] == 'Child').toList();
  ...
  @override
  Future<List<MedicalHistoryModel>> getChildMedicalHistory(String childId) async {
    try {
      final response = await dio.post(
